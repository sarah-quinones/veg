...
#define VEG_IMPL_AND(r, _, param) &&param
#define VEG_IMPL_OR(r, _, param) || param
...
#define VEG_DEF_CONCEPT_CONJUNCTION(tpl, name, terms)                          \
	VEG_DEF_CONCEPT(tpl, name, VEG_CONJUNCTION terms)
#define VEG_DEF_CONCEPT_DISJUNCTION(tpl, name, terms)                          \
	VEG_DEF_CONCEPT(tpl, name, VEG_DISJUNCTION terms)
#define VEG_DEF_CONCEPT_BOOL_CONJUNCTION(tpl, name, terms)                     \
	VEG_DEF_CONCEPT(tpl, name, VEG_CONJUNCTION terms)
#define VEG_DEF_CONCEPT_BOOL_DISJUNCTION(tpl, name, terms)                     \
	VEG_DEF_CONCEPT(tpl, name, VEG_DISJUNCTION terms)

#define VEG_TO_CONCEPT(...) (__VA_ARGS__::value)
#define VEG_ALL_OF(...) ((__VA_ARGS__) && ...)
#define VEG_IMPL_REQUIRES_HANDLER_ONE(...) (__VA_ARGS__)

#define VEG_CONJUNCTION(First, ...)                                            \
	(First __VEG_PP_SEQ_FOR_EACH(                                                \
			VEG_IMPL_AND, _, __VEG_PP_VARIADIC_TO_SEQ(__VA_ARGS__)))

#define VEG_DISJUNCTION(First, ...)                                            \
	(First __VEG_PP_SEQ_FOR_EACH(                                                \
...
#define VEG_IMPL_STRUCT(param) __VEG_PP_CAT(_, __VEG_PP_REMOVE_PAREN(param))
#else
#define VEG_CONCEPT(...) ::veg::concepts::__VA_ARGS__::value
#define VEG_IMPL_ADD_VALUE(r, _, param) ((__VEG_PP_REMOVE_PAREN(param)::value))
#define VEG_IMPL_STRUCT(param) , __VEG_PP_REMOVE_PAREN(param)
#endif
#define VEG_IMPL_REM_PAREN(r, _, param) , VEG_IMPL_STRUCT(param)
...
#define VEG_IMPL_SFINAE(r, _, param)                                           \
	, typename ::veg::meta::enable_if<(param), int>::type = 0
...
#define VEG_DEF_CONCEPT_BOOL_CONJUNCTION_IMPL(tpl, name, conj, seq)            \
	VEG_IMPL_DEF_CONCEPT(                                                        \
			tpl,                                                                     \
			name,                                                                    \
			(__VEG_PP_REMOVE_PAREN1(conj)::value),                                   \
			__VEG_PP_REMOVE_PAREN1(conj));                                           \
	template <__VEG_PP_REMOVE_PAREN(tpl)                                         \
	              __VEG_PP_SEQ_FOR_EACH(VEG_IMPL_SFINAE, _, seq)>                \
	void check_##name() noexcept
#define VEG_DEF_CONCEPT_BOOL_DISJUNCTION_IMPL(tpl, name, disj, terms)          \
	VEG_IMPL_DEF_CONCEPT(                                                        \
			tpl,                                                                     \
			name,                                                                    \
			(__VEG_PP_REMOVE_PAREN1(disj)::value),                                   \
			__VEG_PP_REMOVE_PAREN1(disj));                                           \
	__VEG_PP_SEQ_FOR_EACH(                                                       \
			VEG_IMPL_OVERLOAD, (name, tpl), __VEG_PP_VARIADIC_TO_SEQ terms)          \
	VEG_NOM_SEMICOLON
...
#define VEG_DEF_CONCEPT_BOOL_CONJUNCTION(tpl, name, terms)                     \
	VEG_DEF_CONCEPT_BOOL_CONJUNCTION_IMPL(                                       \
			tpl,                                                                     \
			name,                                                                    \
			::veg::meta::bool_constant<::veg::meta::all_of({terms})>,                \
			__VEG_PP_VARIADIC_TO_SEQ terms)
#define VEG_DEF_CONCEPT_CONJUNCTION(tpl, name, terms)                          \
	VEG_DEF_CONCEPT_BOOL_CONJUNCTION_IMPL(                                       \
			tpl,                                                                     \
			name,                                                                    \
...
#define VEG_DEF_CONCEPT_BOOL_DISJUNCTION(tpl, name, terms)                     \
	VEG_DEF_CONCEPT_BOOL_DISJUNCTION_IMPL(                                       \
			tpl,                                                                     \
			name,                                                                    \
			::veg::meta::bool_constant<::veg::meta::any_of({terms})>,                \
			__VEG_PP_VARIADIC_TO_SEQ terms)
#define VEG_DEF_CONCEPT_DISJUNCTION(tpl, name, terms)                          \
	VEG_DEF_CONCEPT_BOOL_DISJUNCTION_IMPL(                                       \
			tpl,                                                                     \
...
#define VEG_TO_CONCEPT(...) __VA_ARGS__
#define VEG_IMPL_REQUIRES_HANDLER_ONE(...) (__VA_ARGS__)

#define VEG_IMPL_CONJUNCTION(First, ...)                                       \
	conjunction<VEG_IMPL_STRUCT(First) __VEG_PP_SEQ_FOR_EACH(                    \
			VEG_IMPL_REM_PAREN, _, __VEG_PP_VARIADIC_TO_SEQ(__VA_ARGS__))>
#define VEG_IMPL_DISJUNCTION(First, ...)                                       \
	disjunction<VEG_IMPL_STRUCT(First) __VEG_PP_SEQ_FOR_EACH(                    \
			VEG_IMPL_REM_PAREN, _, __VEG_PP_VARIADIC_TO_SEQ(__VA_ARGS__))>
#define VEG_CONJUNCTION(...) VEG_IMPL_CONJUNCTION(__VA_ARGS__)::value
#define VEG_DISJUNCTION(...) VEG_IMPL_DISJUNCTION(__VA_ARGS__)::value
...
#define VEG_IMPL_TEMPLATE(attr_name, tparams, requirement, args)               \
	VEG_IMPL_TEMPLATE2(                                                          \
			...
			__VEG_PP_CAT2(                                                           \
					...
					__VEG_PP_CAT2(VEG_IMPL_PREFIX_, requirement) __VEG_PP_RPAREN()),     \
			__VEG_PP_SEQ_HEAD(args),                                                 \
			__VEG_PP_SEQ_TAIL(args))

#define VEG_IMPL_PREFIX_requires _ONE __VEG_PP_LPAREN()
...
#define VEG_SAME_AS(T, U)                                                      \
	VEG_HAS_BUILTIN_OR(                                                          \
			...
			((VEG_CONCEPT(                                                           \
					same<__VEG_PP_REMOVE_PAREN(T), __VEG_PP_REMOVE_PAREN(U)>))))

#define VEG_DEFAULTS(class_name)                                               \
	~class_name() = default;                                                     \
